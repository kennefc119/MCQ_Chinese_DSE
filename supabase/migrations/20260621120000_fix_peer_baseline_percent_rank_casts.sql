-- Fix: percent_rank() returns double precision; round(..., 1) requires numeric.
-- Recreate admin_refresh_peer_baselines with numeric casts on percentile outputs.
  ...
  select
    ...
    jsonb_build_object(
      'points', jsonb_build_object(
        ...
        'percentile', round((coalesce(r.pr_points, 0) * 100.0)::numeric, 1)
      ),
      'accuracy', jsonb_build_object(
        'value', round(r.accuracy, 1),
        'percentile', round((coalesce(r.pr_accuracy, 0) * 100.0)::numeric, 1)
      ),
      'completed_questions', jsonb_build_object(
        'value', round(r.completed_questions, 1),
        'percentile', round((coalesce(r.pr_questions, 0) * 100.0)::numeric, 1)
      ),
      'completed_quizzes', jsonb_build_object(
        'value', round(r.completed_quizzes, 1),
        'percentile', round((coalesce(r.pr_quizzes, 0) * 100.0)::numeric, 1)
      )
    )
  from ranks r;
