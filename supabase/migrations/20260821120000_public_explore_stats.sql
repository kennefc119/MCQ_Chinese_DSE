-- Public Explore aggregates.
-- Exposes only passage and quiz-level statistics; no attempt or user rows.

create index if not exists idx_attempts_explore_stats
  on dsemcq_attempts(quiz_id, user_id)
...
passage_counts as (
  select
    p.id,
    count(distinct l.question_id)::int as active_question_count
  from dsemcq_passages p
  left join active_passage_question_links l on l.passage_id = p.id
  group by p.id
),
active_quizzes as (
