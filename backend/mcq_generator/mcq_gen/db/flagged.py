...
def fetch_deactivated_questions(
    *,
    question_id: str | None = None,
) -> list[FlaggedQuestion]:
    """
    Fetch deactivated questions (is_active=false) for the correction workflow.

    Reuses the FlaggedQuestion dataclass — user_flag fields will be 0/empty
    for questions that were admin-deactivated rather than user-flagged.
    """
    sb = get_supabase()
    ...
    log.info("deactivated_questions_fetched", count=len(results))
    return results
